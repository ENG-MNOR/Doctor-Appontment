...
class diagnose extends DatabaseConnection
{
    ...
    public  function createDiagnose($_conn)
    {
        ...
        $sql = "INSERT into diagnose(`name`,`description`) VALUES ('$name','$decription')";
        if (!$_conn)
    ...
    public  function deleteDiagnose($_conn)
    {
        ...
        $sql = "DELETE FROM diagnose where diganose_id='$id';";
        if (!$_conn)
            $response = array("error" => "There is an error connection ", "status" => false);
        else {
            try {
    ...
    public function updateDiagnose($conn){
        ...
        $sql = "UPDATE diagnose set `name` = $name, 'description' = $description WHERE diganose_id = $id";
        if(!$conn){
