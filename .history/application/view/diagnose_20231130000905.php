    ...
    const readDiagnose = () => {
        $.ajax({
            ...
            success: (res) => {
                ...
                var { data } = res;
                data.forEach(value => {
                    tr += `<td>${value.digonose_id}</td>`
                    tr += `<td>${value.name}</td>`
                    tr += `<td>${value.description}</td>`
                    tr += `<td><a class='btn btn-success editButton' editID=${value.digonose_id}>Edit</a>
                      <a class='btn btn-danger deleteDiagnose' delID=${value.digonose_id}>Delete</a></td>`
                    tr += '</tr>'
                })
                $(".table tbody").html(tr)
                console.log(data)
