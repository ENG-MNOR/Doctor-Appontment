<?php session_start();
?>


<div class="quixnav">
            <div class="quixnav-scroll">
                <ul class="metismenu" id="menu">
                    <li class="nav-label first text-light">Super Admin </li>
                    <li><a  href="../view/dashboard.php" aria-expanded="false">
                    <i class="fa-solid fa-gauge"></i><span class="nav-text">Dashboard</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>
                    </div></div>
  
    
    <!--**********************************
            Sidebar start
        ***********************************-->
        <div class="quixnav">
            <div class="quixnav-scroll">
                <ul class="metismenu" id="menu">
                    <li class="nav-label first text-light">Super Admin</li>
                    <li><a  href="../view/dashboard.php" aria-expanded="false">
                    <i class="fa-solid fa-gauge"></i><span class="nav-text">Dashboard</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>
                    <li><a  href="../view/doctors.php" aria-expanded="false">
                    <i class="fa-solid fa-user-doctor"></i><span class="nav-text">Doctors</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>   
                    <li><a  href="../view/admin.php" aria-expanded="false">
                    <i class="fa-solid fa-user-pen"></i><span class="nav-text">Admins</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>  <li><a href="../view/hospital.php" aria-expanded="false">
                        <i class="fa-solid fa-hospital"></i></i><span class="nav-text">Hospitals</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>   <li><a  href="../view/patient.php" aria-expanded="false">
                    <i class="fa-solid fa-suitcase-medical"></i><span class="nav-text">Patients</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>   <li><a  href="../view/profision.php"aria-expanded="false">
                    <i class="fa-solid fa-user-tie"></i><span class="nav-text">Profision</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>   <li><a  href="../view/schedule.php" aria-expanded="false"><i class="fa-solid fa-calendar-days"></i><span class="nav-text">Schedule</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>   <li><a  href="../view/diagnose.php" aria-expanded="false">
                    <i class="fa-solid fa-capsules"></i><span class="nav-text">Diagnose</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>   <li><a  href="../view/apointment.php" aria-expanded="false">
                    <i class="fa-solid fa-calendar-check"></i><span class="nav-text">Apointment</span></a>
                        <!-- <ul aria-expanded="false">
                            <li><a href="../.././index.html">Dashboard 1</a></li>
                            <li><a href="../.././index2.html">Dashboard 2</a></li>
                        </ul> -->
                    </li>

                </ul>
            </div>


        </div>
        <!--**********************************
            Sidebar end
        ***********************************-->